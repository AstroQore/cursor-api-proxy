...
- **Server-side workspace (optional):** The Cursor CLI may run with a workspace directory (`CURSOR_BRIDGE_WORKSPACE`). By default, `CURSOR_BRIDGE_CHAT_ONLY_WORKSPACE=true` runs the CLI in an **empty temp directory** so it does not read or write your real project; the proxy also overrides `HOME`, `USERPROFILE`, and `CURSOR_CONFIG_DIR` so the agent does not load global or project rules from elsewhere. Per-request workspace hints are ignored unless `CURSOR_BRIDGE_ALLOW_WORKSPACE_HINTS=true`. Set chat-only to `false` only if you intentionally want the CLI to see a path on the machine where the proxy runs (still not the same as IDE indexing—see env table below).
...
| `CURSOR_BRIDGE_TRUST_CHAT_ONLY_WORKSPACE` | `true` | When `false`, do not pass `--trust` for isolated chat-only workspaces. This helps deployments that expose Cursor as a normal API provider avoid accidental reads of the bridge's temporary workspace. |
| `CURSOR_BRIDGE_EMPTY_CHAT_ONLY_WORKSPACE` | `false` | When `true`, do not write `.cursor` / `.config` scaffolding into the isolated temporary workspace. Use with account-pool auth for the cleanest API-provider behavior. |
| `CURSOR_BRIDGE_ALLOW_WORKSPACE_HINTS` | `false` | When `true`, honor non-standard request workspace hints (`X-Cursor-Workspace`, `cwd`, `workspace`, `working_directory`, and matching `metadata.*` fields). Leave `false` for normal API-provider deployments. |
| `CURSOR_BRIDGE_PROMPT_FORMAT` | `transcript` | `transcript` preserves the original `System/User/Assistant:` flattening. `minimal` sends a single user message as raw text and uses bracketed role blocks only for multi-turn prompts, closer to `cursor-agent-api-proxy` behavior. |
...
- Header **`X-Cursor-Workspace: <path>`** — when `CURSOR_BRIDGE_ALLOW_WORKSPACE_HINTS=true`, use a subdirectory of `CURSOR_BRIDGE_WORKSPACE` (requires real workspace: set `CURSOR_BRIDGE_CHAT_ONLY_WORKSPACE=false` or use a non-`ask` mode without forcing chat-only; path must exist on the proxy host).
- JSON body **`cwd`**, **`workspace`**, **`working_directory`**, **`metadata.cwd`**, or **`metadata.workspace`** — same as `X-Cursor-Workspace` when `CURSOR_BRIDGE_ALLOW_WORKSPACE_HINTS=true`. Leave workspace hints disabled for normal OpenAI-compatible provider deployments.
