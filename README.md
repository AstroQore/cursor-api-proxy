...
- JSON body **`cwd`**, **`workspace`**, **`working_directory`**, **`metadata.cwd`**, or **`metadata.workspace`** — same as `X-Cursor-Workspace`. This is useful for OpenAI-compatible clients that can pass `extra_body` but not custom headers. An explicit workspace overrides chat-only temp workspace for that request and is trusted for non-interactive CLI execution.
